<?php
namespace App\Modules\Ventes\controllers;
...
class Ventes extends BaseController
{
  ...
  public function get_view()
  {
    ...
    return view('App\Modules\Ventes\Views\Ventes_Add_View',$data);
  }
  ...
  function getListe()
  {
    ...
    $get_data = $db->query("SELECT s.QTE,s.DATE_INSERTION,s.PU,s.QTE*s.PU as PT,s.IS_DETTE,p.DESC_PRODUIT,p.NBR_BOUTEILLE_PAR_CASIER,r.DESC_RAISON_SORTIE FROM sortie s JOIN produits_lot pl ON pl.ID_PRODUIT_LOT=s.ID_PRODUIT_LOT JOIN produits p ON p.ID_PRODUIT=pl.ID_PRODUIT JOIN raison_sortie r ON r.ID_RAISON_SORTIE=s.ID_RAISON_SORTIE WHERE 1");
    $data["get_data"] = $get_data->getResult();

    return view('App\Modules\Ventes\Views\Ventes_List_view',$data);
  }
}
?>